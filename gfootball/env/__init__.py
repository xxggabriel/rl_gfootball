...
def _process_reward_wrappers(env, rewards, agent_on_left=True):
  assert 'scoring' in rewards.split(',')
  if 'checkpoints' in rewards.split(','):
    env = wrappers.CheckpointRewardWrapper(env)
  if 'shaping' in rewards.split(','):
    env = wrappers.DenseRewardWrapper(env, agent_on_left)
  return env
...
def _apply_output_wrappers(env, rewards, representation, channel_dimensions,
                           apply_single_agent_wrappers, stacked, agent_on_left=True):
  ...
  Returns:
    Google Research Football environment.
  """
  env = _process_reward_wrappers(env, rewards, agent_on_left)
...
def create_environment(env_name='',
  ...
  Args:
    ...
    rewards: Comma separated list of rewards to be added.
       Currently supported rewards are 'scoring', 'checkpoints' and 'shaping'.
  ...
  if dump_frequency > 1:
    ...
    env.render()
  agent_on_left = (
      number_of_right_players_agent_controls == 0 or
  ...
  env = _apply_output_wrappers(
      ...
      (number_of_left_players_agent_controls +
       number_of_right_players_agent_controls == 1), stacked, agent_on_left)
  return env
...
def create_remote_environment(
  ...
  agent_on_left = env._config.number_of_left_players() >= env._config.number_of_right_players()
  env = _apply_output_wrappers(
      env, rewards, representation, channel_dimensions,
      env._config.number_of_players_agent_controls() == 1, stacked, agent_on_left)
  return env
